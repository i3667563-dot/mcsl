...
impl Compiler {
    ...
    fn write_datapack(&self, codegen: &CodeGenerator) -> Result<(), CompilerError> {
        ...
        let functions_dir = output_dir.join("data").join(&self.config.namespace).join("function");
        let ifblocks_dir = functions_dir.join("ifBlocks");
        let tags_dir = output_dir.join("data").join("minecraft").join("tags").join("function");
...
pub fn compile_file(input_path: &Path, output_dir: &Path, namespace: &str) -> Result<(), CompilerError> {
